...
###### take a subset of tasks for training, consider all the rest for testing only (include their logreg models in VectorDB)
np.random.seed(42)
subset_task_indices = np.random.randint(0, 4412, size=2_000) ## 4412 tasks have at least 2 training samples.
# subset_task_indices = np.setdiff1d(np.arange(4412), subset_task_indices)
X_tests_1 = [X_tests_1[i] for i in subset_task_indices]
y_tests_1 = [y_tests_1[i] for i in subset_task_indices]
bert_class_embeddings = bert_class_embeddings[subset_task_indices]
logRegModel_weights = logRegModel_weights[subset_task_indices]
assert len(X_tests_1) == len(y_tests_1) == len(bert_class_embeddings)
